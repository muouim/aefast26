...
DBImpl_Sharding::DBImpl_Sharding(const Options& options, const std::string& dbname) {
    assert(options.StringShardInfo->size() != 0);
    for (auto iter : *options.StringShardInfo) {
      Shard_Info.emplace_back(iter.first, iter.second);
    }
...
Iterator* DBImpl_Sharding::NewIterator(const ReadOptions& options, const Slice& key) {

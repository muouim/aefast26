...
base_dir="$PWD"
status_file="/tmp/ycsb_exp.flag"
output_path="$base_dir/ycsb.output"
...
if [ -f "$status_file" ]; then
    if grep -q "running" "$status_file"; then
        ...
        echo "YCSB experiment is complete, no need to run the script again."
        exit 0
    fi
else
...
# Phase 1: Run YCSB experiment for each baseline
echo "---------- Phase 1: Run YCSB experiment for each baseline ----------"

for project in $baselines; do
    echo "============================="
    echo "Running experiment for $project"
    script_path="$base_dir/$project/script/run_exp14.sh"
    ...
    python3 $base_dir/AE/print_exp14.py
done
...
python3 $base_dir/AE/print_exp14.py
python3 $base_dir/AE/plot_exp14.py

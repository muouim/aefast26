...
const char* RdmaKeeper::OK = "OK";
const char* RdmaKeeper::ServerPrefix = "SPre";

void RdmaKeeper::initLocalMeta() {
...
bool RdmaKeeper::connectNode(uint16_t remoteID) {
...
// send threadconnection qp rkey
void RdmaKeeper::setDataToRemote(uint16_t remoteID) {
	for(int i = 0; i < NR_DIRECTORY; ++i) {
		auto& c = dirCon[i];
...
// get dirconnection qp rkey
void RdmaKeeper::setDataFromRemote(uint16_t remoteID, ExchangeMeta* remoteMeta) {
	for(int i = 0; i < NR_DIRECTORY; ++i) {
		auto& c = dirCon[i];
	...
	for(int i = 0; i < NR_DIRECTORY; ++i) {
		...
		for(int k = 0; k < MAX_THREAD_NUM; ++k) {
			...
			fillAhAttr(&ahAttr, remoteMeta->dirTh[i].lid,
			           remoteMeta->dirTh[i].gid, &thCon[k]->ctx);
	...
	for(int i = 0; i < MAX_THREAD_NUM; ++i) {
		...
		for(int k = 0; k < NR_DIRECTORY; ++k) {
			struct ibv_ah_attr ahAttr;
			fillAhAttr(&ahAttr, remoteMeta->appTh[i].lid,
			           remoteMeta->appTh[i].gid, &dirCon[k]->ctx);
...
void RdmaKeeper::barrier(const std::string& barrierKey) {
...
uint64_t RdmaKeeper::sum(const std::string& sum_key, uint64_t value) {

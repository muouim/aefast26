...
struct IdxCacheEntry {
  ...
  IdxCacheEntry() : leaf_addr(GlobalAddress::Null()), kv_idx(0), fingerprint(0), cache_entry_freq(0) {}
...
private:
  ...
  std::mutex bucket_lock[HASH_TABLE_SIZE];
  DSM *dsm;
  bool is_disabled;
...
inline bool IdxCache::add_to_cache(const GlobalAddress& leaf_addr, int kv_idx, const Key& k) {
  ...
  for (const auto& e : bucket) {
    if(e.leaf_addr == GlobalAddress::Null()) {
        continue;
    }
    if (e.leaf_addr == leaf_addr && e.kv_idx == kv_idx) {
  ...
  for (int i = 0; i < BUCKET_SIZE; ++ i) {
    auto& e = bucket[i];
    if (e.leaf_addr == GlobalAddress::Null()) {
        e.leaf_addr = leaf_addr;
        e.kv_idx = kv_idx;
        e.fingerprint = key2fp(k);
        e.cache_entry_freq = 1;
        auto v = free_size.fetch_add(-sizeof(IdxCacheEntry));
        if (v < 0) {
          evict();
        }
        bucket_lock[ht_idx].unlock();
        return true;
    }
...
inline bool IdxCache::search_idx_from_cache(const GlobalAddress& leaf_addr, int l_idx, int r_idx, const Key &k, int& kv_idx) {
  ...
  thread_local int32_t max_cnt = -1;
  thread_local int32_t max_idx = -1;
  if (is_disabled) {
    return false;
  }
  ...
  auto search_for_candidate = [&](int idx){
    ...
    for (const auto e : bucket) {
        ...
        if (e.leaf_addr == leaf_addr && e.kv_idx == idx) {
    #ifdef FOOTPRINT_SPECULATIVE_READ
        if (e.fingerprint != k_fp) continue;
    #endif

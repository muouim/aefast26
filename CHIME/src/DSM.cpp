...
uint64_t rdma_cnt[MAX_APP_THREAD][MAX_MACHINE];
uint64_t rdma_bw[MAX_APP_THREAD][MAX_MACHINE];
...
void DSM::cas_dm_mask(GlobalAddress gaddr, uint64_t equal, uint64_t val,
  ...
  if (sink == nullptr) {
    rdmaCompareAndSwapMask(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                          ...
                          remoteInfo[gaddr.nodeID].lockRKey[0], mask, mask, signal);
  }
  else {
    rdmaCompareAndSwapMask(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer,
                          remoteInfo[gaddr.nodeID].lockBase + gaddr.offset,
                          equal, val, iCon->cacheLKey,
                          remoteInfo[gaddr.nodeID].lockRKey[0], mask, mask, true, sink->get());
    (*sink)();
  }
}

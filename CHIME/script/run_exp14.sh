    ...
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                ...
                ssh skv-node$n "cd $dm_tree_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_chime_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            ...
            for n in $node; do
                ...
                scp "skv-node$n:$dm_tree_dir/data/node$n-exp0_chime_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done

...
class InlineSkipList {
 ...
 public:
  ...
  // is thread-safe.
  char* AllocateKey(size_t key_size, int &height, char **free_raw);
 ...
 private:
  ...
  Node* AllocateNode(size_t key_size, int &height, char **free_raw);
...
template <class Comparator>
char* InlineSkipList<Comparator>::AllocateKey(size_t key_size, int &height, char **free_raw) {
  return const_cast<char*>(AllocateNode(key_size, height, free_raw)->Key());
}

template <class Comparator>
typename InlineSkipList<Comparator>::Node*
InlineSkipList<Comparator>::AllocateNode(size_t key_size, int &height, char **free_raw) {
  if(*free_raw == nullptr) {
    height = RandomHeight();
  }
  ...
  // the Node.
  if(*free_raw == nullptr) {
    *free_raw = allocator_->AllocateAligned(prefix + sizeof(Node) + key_size);
  } 
  Node* x = reinterpret_cast<Node*>(*free_raw + prefix);

...
#ifdef TREE_ENABLE_EMBEDDING_LOCK
    return dsm->cas_sync(GADD(unique_leaf_addr, lock_cas_offset), 0UL, ~0UL, cas_buffer, cxt);
#else
    GlobalAddress lock_addr;
    uint64_t mask;
    get_on_chip_lock_addr(unique_leaf_addr, lock_addr, mask);
    return dsm->cas_dm_sync(lock_addr, 0UL, ~0UL, cas_buffer, cxt);
#endif
  };
...
#ifdef TREE_ENABLE_EMBEDDING_LOCK
    dsm->cas_sync(GADD(unique_leaf_addr, lock_cas_offset), ~0UL, 0UL, cas_buffer, cxt);
#else
    GlobalAddress lock_addr;
    uint64_t mask;
    get_on_chip_lock_addr(unique_leaf_addr, lock_addr, mask);
    dsm->cas_dm_sync(lock_addr, ~0UL, 0UL, cas_buffer, cxt);
#endif
  };
...
re_acquire:
  if (!acquire_lock(leaf_addr)){
    if (cxt != nullptr) {
      cxt->busy_waiting_queue->push(std::make_pair(coro_id, [](){ return true; }));
...
write_leaf:
  ...
  dsm->write_cas_sync(rs[0], rs[1], ~0UL, 0UL, cxt);
#endif
#endif
  return;
}
...
void Tree::cas_node_type(NodeType next_type, GlobalAddress p_ptr, InternalEntry p, Header hdr,
  ...
  auto remote_cas_header = [=, &hdr](){
    return dsm->cas_sync(header_addr, hdr, Header(next_type, hdr), cas_buffer_2, cxt);
  };
  ...
  for(auto & s : survivors) {
    ...
    if (tokens.find(token) == tokens.end()) {
      ...
      r.size       = p.is_leaf ? std::max((unsigned long)p.kv_len, sizeof(Leaf)) : (
                              ...
                              (sizeof(GlobalAddress) + sizeof(Header) + node_type_to_num(NODE_16) * sizeof(InternalEntry)) :

...
next:
  ...
  for (int i = 0; i < (int)hdr->partial.size(); ++ i) {
    ...
    if (hdr->depth + i == (int)byte_array.size() - 1 || cur_partial != hdr->partial[i]) {
      ...
      auto& parent_node_entry = parent_node->records.search_init(byte_array[idx - 1]);
      ...
      // auto& tmp = (nested_node ? nested_node->records.search(byte_array.back()) : new_node->records.search(cur_partial));
      // eviction_list.push(std::make_pair(&(tmp.cache_entry), new_entry));
      return;
    }
  }
  ...
  if (idx == (int)byte_array.size() - 1) {
    ...
    auto& node_entry = cache_map.search_init(partial);
  ...
  // 2.2 internal level
    ...
    if (node_entry.next == nullptr) {
      ...
      if (ret_node == 0UL) {  // cas success
        ...
        // eviction_list.push(std::make_pair(&(tmp->second.cache_entry), new_entry));
        // auto& tmp = next_node->records.search(byte_array.back());
        // eviction_list.push(std::make_pair(&(tmp.cache_entry), new_entry));
...
next:
  ...
  bool is_found = false;
  auto& r_entry = cache_map.search(partial, is_found);
  if (is_found) {
    auto cache_entry = (CacheEntry *)r_entry.cache_entry;
    // ret.push(std::make_pair(std::make_pair(&(r_entry->second.cache_entry), cache_entry), idx + 1));
    ret.push(SearchRet(&(r_entry.cache_entry), cache_entry, idx + 1));
    node = (CacheNode *)(r_entry.next);
    if (node) {
      idx ++;
      goto next;
    }
...
void RadixCache::search_range_from_cache(const Key &from, const Key &to, std::vector<RangeCache> &result) {
  ...
  auto from_int = std::stol(from_key.c_str());
  auto to_int = std::stol(to_key.c_str());
  // std::cout<<"from: "<<from_int<<", to: "<<to_int<<std::endl;
  
  for (auto k_int = from_int; k_int < to_int; k_int = k_int + 1) {
    auto str = std::to_string(k_int);
    auto k = str2key(std::string(define::keyLen - str.size(), '0') + str);
...
void RadixCache::_evict() {
  ...
  if (flag) {
    ...
    std::cout<<"need rebuild"<<std::endl;
...
void RadixCache::clear() {
  ...
  node_queue = new tbb::concurrent_queue<CacheNode*>();
  node_queue->push(cache_root);
  while (cache_node_gc.unsafe_size() > 0) {
    CacheNode* next = nullptr;
    if (cache_node_gc.try_pop(next) && next) {
      delete next;
    }
  }
}
...
void RadixCache::statistics() {
  ...
  for (auto node_iter = node_queue->unsafe_begin(); node_iter != node_queue->unsafe_end(); ++ node_iter) {
    ...
    total_cache += node->content_size();
    for (int i = 0; i < 10; i++) {
      auto cache_entry = (CacheEntry *)cache_map.value[i].cache_entry;
      if (cache_entry) {
        int depth = cache_entry->depth;
        if (cnt.find(depth) == cnt.end()) cnt[depth] = 0;
        cnt[depth] ++;
        kp_cnt += cache_entry->records.size();
        total_cache += cache_entry->content_size();
      }
    }
  }
  ...
  std::cout << "cache used=" << total_cache / define::MB << " MB" << std::endl;

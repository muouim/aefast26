...
public:
  ...
  bool is_consistent() const {
	...
	auto crc = CityHash32((char *)&key, sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
    return crc == checksum;
  }
  ...
  void set_consistent() {
    ...
    checksum = CityHash32((char *)&key, sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
  }
...
public:
  ...
  Header(NodeType node_type, const Header& old_hdr) : depth(old_hdr.depth), node_type(node_type), partial_len(old_hdr.partial_len) { 
    memset(partial, 0, sizeof(uint8_t) * define::hPartialLenMax); 
    for (int i = 0; i < old_hdr.partial_len; ++ i) partial[i] = old_hdr.partial[i];
  }
...
public:
  ...
  Header hdr;
  InternalEntry records[16];

public:
  InternalPage() { std::fill(records, records + 16, InternalEntry::Null()); }
  InternalPage(const Key &k, int partial_len, int depth, NodeType node_type, const GlobalAddress& rev_ptr) : rev_ptr(rev_ptr), hdr(k, partial_len, depth, node_type) {
    std::fill(records, records + 16, InternalEntry::Null());
  }
...
static_assert(sizeof(InternalPage) == 8 + 8 + 16 * 8);

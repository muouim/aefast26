...
public: 
    ...
    CacheNodeValue& search_init(uint8_t k) {
        if(k == 0) {
            return null;
        }
        ...
        for(int i = 0; i < 10; i++) {
            ...
            if(map[i] == 0) {
                if(__sync_val_compare_and_swap(&map[i], 0, k)) {
                    return value[i];
                } else {
                    i--;
                }
    ...
    CacheNodeValue& search(uint8_t k) {
        if(k == 0) {
            return null;
        }
    ...
    CacheNodeValue& search(uint8_t k, bool &is_found) {
        if(k == 0) {
            return null;
        }
        // std::shared_lock<std::shared_mutex> lock(mutex_);
    ...
    void insert(uint8_t k, const CacheNodeValue &v) {
        if(k == 0) {
            return;
        }
        // std::unique_lock<std::shared_mutex> lock(mutex_);
        for(int i = 0; i < 10; i++) {
            if(map[i] == k) {
                value[i].cache_entry = v.cache_entry;
                value[i].next = v.next;
                break;
            }
            if(map[i] == 0) {
...
public:
  ...
  CacheNode(const std::vector<uint8_t>& byte_array, int start, CacheEntry* new_entry) {
    ...
    records.insert(byte_array.back(), CacheNodeValue(new_entry, nullptr));
  }
  ...
  CacheNode(const std::vector<uint8_t>& byte_array, int start, int partial_len,
    ...
    if (partial_1 == partial_2) {  // split for insert new_entry at old header
      ...
      records.insert(partial_1, CacheNodeValue(new_entry, next_node));
    }
    else {
      // records[partial_1] = CacheNodeValue(nullptr, next_node);
      records.insert(partial_1, CacheNodeValue(nullptr, next_node));
      if (start + partial_len >= (int)byte_array.size() - 1) {  // insert entry directly
        nested_node = nullptr;
        // records[partial_2] = CacheNodeValue(new_entry, nullptr);
        records.insert(partial_2, CacheNodeValue(new_entry, nullptr));
      }
        ...
        records.insert(partial_1, CacheNodeValue(nullptr, nested_node));
      }
    }
  }

  uint64_t content_size() {
    return ((CacheHeader *)header)->content_size() + (sizeof(uint8_t) + sizeof(CacheEntry *) + sizeof(CacheNode *)) * 10;
  }
